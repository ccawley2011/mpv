...
int vo_sdl_init(struct vo *vo, int flags)
{
    ...
    *sdl = (struct vo_sdl_state){
        ...
        .opts_cache = m_config_cache_alloc(sdl, vo->global, &vo_sub_opts),
    };
    sdl->opts = sdl->opts_cache->opts;
    vo->sdl = sdl;
...
static void set_fullscreen(struct vo *vo)
{
    ...
    struct mp_vo_opts *opts = sdl->opts_cache->opts;
...
{
    struct vo_sdl_state *sdl = vo->sdl;
    struct mp_vo_opts *opts = sdl->opts;
    switch (request) {
    ...
    case VOCTRL_VO_OPTS_CHANGED: {
        void *opt;
        while (m_config_cache_get_next_changed(sdl->opts_cache, &opt)) {
